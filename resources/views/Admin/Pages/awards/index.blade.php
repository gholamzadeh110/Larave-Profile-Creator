...
@section('title', __('messages.Awards'))
@section('page-style')
...
@section('content')
    <div class="row clearfix">
        <div class="col-lg-12">
            <div class="card">
                <div class="header">
                    <h2><strong><i class="zmdi zmdi-chart"></i> {{ __('messages.Add_New_Awards') }}</strong></h2>
                ...
                <div class="body mb-2">
                    <form enctype="multipart/form-data" method="POST" action="{{ route('awards.store') }}">
                        @csrf
                        ...
                        <div class="card col-12">
                            ...
                            <div class="card-footer text-right">
                                ...
                                <button class="btn btn-secondary" href="{{ route('awards.create') }}" type="reset">{{ __('messages.refresh') }}</button>
                            </div>
                        </div>
                    </form>
                </div>
...
<div class="row clearfix">
    <div class="col-lg-12">
        <div class="card">
            <div class="header">
                <h2><strong><i class="zmdi zmdi-chart"></i> {{ __('messages.Awards') }}</strong></h2>
                <ul class="header-dropdown">
                    <li class="remove">
                        <a role="button" class="boxs-close"><i class="zmdi zmdi-close"></i></a>
                    </li>
                            ...
                            <div class="col-12">
                                <table class="table table-striped table-hover" id="save-stage" style="width:100%;"  style="font-size: 12px">
                                    ...
                                    @foreach ($awards as $award)
                                        <tr class="row1" data-index="{!! $award->id !!}">
                                            <td class="align-middle">
                                                {{ Str::limit( $award->description, 100) }}
                                            </td>
                                            <td class="align-middle">
                                                <?php $v = new Verta($award->created_at); ?>

                                                {{ $v->format('j %B Y') }}
                                            </td>
                                            <td class="align-middle">
                                                <?php $v1 = new Verta($award->updated_at); ?>

                                                {{ $v1->format('j %B Y') }}
                                            </td>
                                            <td class="align-middle">
                                                <form action="{{ route('awards.destroy', ['award' => $award->id]) }}" method="POST">
                                                    @csrf
                                                    <input type="hidden"  id="id" name="id" value="{{$award->id}}">
